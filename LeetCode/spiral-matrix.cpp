...
private:
    ...
    void scanRightToLeft(vector<vector<int> > &matrix, vector<int> &result, int round) {
        //There are more than 1 row in currrent round to traverse, aka. 2 * (round - 1) + 1 < matrix.size()
		if (2 * (round - 1) + 1 < matrix.size()) {
    ...
    void scanBottomToTop(vector<vector<int> > &matrix, vector<int> &result, int round) {
        //There are more than 1 col in currrent round to traverse, i.e. 2 * (round - 1) + 1 < matrix[0].size()
		if (2 * (round - 1) + 1 < matrix[0].size()) {
